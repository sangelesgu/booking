...
const getDataFromStarChamps = async (checkIn = '01 jul 2021', checkOut = '16 jul 2021') => {
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      $roomsRate.forEach(($room) => {
        info.push({
          minPrice: $room.querySelector('.room-rates-item-price-moy').textContent.replace('€', '').trim(),
